<div class="container">
  <div class="row">
    <div class="col-12">
      <h1 class="display-1 text-center mb-4">
        Jorge Ares Pons
      </h1>
      <p class="lead">
        BREVE RESEÑA DE LA ACTIVIDAD UNIVERSITARIA DE JORGE ARES PONS
      </p>
      <ol class="list-unstyled">
        <li>
          Ingreso a la UDELAR  a mediados de la década del 40 (estudiante)
        </li>
        <li>
          Orden Docente (desde Becario de Investigación a Profesor adjunto de Dedicación Total (1955 hasta destitución en 1975)
        </li>
        <li>
          Delegado Profesional al Claustro de la Facultad de Química (1959-1965)
        </li>
        <li>
          Idem  Docente (1965-1969)
        </li>
        <li>
          Doctorado en Química de la UDELAR   (1970)
        </li>
        <li>
          Delegado Docente al Claustro General (1967-1975)
        </li>
        <li>
          Consejero Docente  de la Facultad de Química (1967-1975)
        </li>
        <li>
          Miembro del CDC por la Facultad de Química (1970-1972)
        </li>
        <li>
          Pro-Secretario y Secretario General de la Asociación de Química y Farmacia del Uruguay (1962-1966)
        </li>
        <li>
          Miembro del Consejo Federal de la Federación de Docentes Universitarios del Uruguay 1969-1970)
        </li>
        <li>
          Académico Correspondiente en el Extranjero de la Academia Nacional de Farmacia y Bioquímica Argentina (ver mención en "Revista Farmacéutica", 151 ,Nº2, 2009)
        </li>
        <li>
          DICTADURA CIVICO MILITAR
        </li>
        <li>
          Actividad encubierta con los  órdenes universitarios
        </li>
        <li>
          POST-DICTADURA
        </li>
        <li>
          Reintegro a la docencia 1985-1989
        </li>
        <li>
          Integrante del CDC provisorio electo en 1984
        </li>
        <li>
          Firmante de los acuerdos de la CONAPRO para la transición universitaria
        </li>
        <li>
          Delegado docente al Claustro General  y Delegado profesional  al Claustro General (1989-1993)
        </li>
        <li>
          Miembro del CDC definitivo  (1985-1991)   I ntegrante de la Mesa Asesora del Rectorado (CDC Provisorio y Definitivo)
        </li>
        <li>
          Asistente Académico de los Rectores Brovetto y Guarga hasta el año 2005
        </li>
        <li>
          Miembro del equipo fundador del PEDECIBA (a propuesta de la UDELAR) desde 1984, e integrante de su Comisión Directiva hasta el año 2005 (veinte años de gestión)
        </li>
        <li>
          Redactor principal del informe previo a la aprobación del Decreto 308/995, regulatorio de las instituciones terciarias privadas : CONSEJO CONSULTIVO DE ENSEÑANZA TERCIARIA PRIVADA (CCETP)
        </li>
        <li>
          Miembro del CCETP, a propuesta dela UDELAR, durante dieciséis años de gestión. Seis últimos años –hasta 2011- como Presidente.
        </li>
        <li>
          OTRAS TAREAS (científicas, docentes y administrativas, llevadas adelante durante los más de sesenta años de dedicación a la UDELAR)Cursos especiales en Montevideo y en el Exterior, trabajos de investigación,  incluso interrumpidos durante la intervención de la UDELAR, trabajos sobre organización universitaria, docencia, formación profesional, legislación farmacéutica, etc.) Ver CURRICULUM VITAE en Subpágina "LIBROS/OTROS" de la PÁGINA PRINCIPAL.
        </li>
      </ol>
      <p>
        Contacto:
        <a href="mailto:sitio.jorge@ares.uy">
          sitio.jorge@ares.uy
        </a>
      </p>
    </div>
  </div>
</div>
